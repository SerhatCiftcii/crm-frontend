...
import { Add as AddIcon, Edit as EditIcon, Delete as DeleteIcon, Visibility as VisibilityIcon, Download as DownloadIcon } from '@mui/icons-material';
    ...
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        ...
        <Box sx={{ display: 'flex', gap: 1 }}>
          <Button variant="contained" color="primary" startIcon={<AddIcon />} onClick={handleAdd}>Yeni Müşteri Ekle</Button>
          <Button variant="outlined" color="secondary" startIcon={<DownloadIcon />} onClick={handleDownloadChangeLog}>
            ...
            <Table stickyHeader sx={{ minWidth: 800 }}>
              ...
              <TableBody>
                {customers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((customer, index) => (
                  <TableRow key={customer.id} hover sx={{ bgcolor: index % 2 === 0 ? 'grey.50' : 'background.paper' }}>
                    <TableCell>{customer.companyName}</TableCell>
                    <TableCell>{customer.ownerName}</TableCell>
                    <TableCell>{customer.email}</TableCell>
                    <TableCell>{customer.phone}</TableCell>
                    <TableCell>{customer.city}</TableCell>
                    <TableCell>{statusMap[customer.status]}</TableCell>
                    <TableCell>
                      {customer.products?.map(p => p.name).join(', ') || '-'}
                    </TableCell>
                    <TableCell align="center">
                      <Tooltip title="Detaylar">
                        <IconButton color="info" onClick={() => navigate(`/customers/${customer.id}`)}><VisibilityIcon /></IconButton>
                      </Tooltip>
                      <Tooltip title="Düzenle">
                        <IconButton color="primary" onClick={() => handleEdit(customer.id)}><EditIcon /></IconButton>
                      </Tooltip>
                      <Tooltip title="Sil">
